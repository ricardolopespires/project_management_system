...
class Documentary(models.Model):
	...
	Rating = models.ManyToManyField(D_Rating, blank=True)
...
class D_Review(models.Model):
	user = models.ForeignKey(settings.AUTH_USER_MODEL, related_name = "documentary", on_delete=models.CASCADE)
...
class D_Likes(models.Model):
	user = models.ForeignKey(settings.AUTH_USER_MODEL , on_delete=models.CASCADE, related_name='documentary_user_like')
	type_like = models.PositiveSmallIntegerField()
	review = models.ForeignKey(D_Review, on_delete=models.CASCADE, related_name='documetary_review_like')