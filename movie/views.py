...
def movie_templateview(request):
	movie_data = Movie.objects.all()


	paginator = Paginator(movie_data, 6) # 3 posts in each page
	...
	return render(request, 'movie/index.html',{'movie_data':movie_data,'page':page, 'paginas':paginas})
...
def movie_search(request):
	...
	if query is not None:
		...
		return render(request, 'movie/search.html',{ 'query': query, 'movie_data': movie_data, 'page_number': 1,	})
...
def movie_updated(request, imdb_id):
...
def movie_delete(request, imdb_id):
    serie = get_object_or_404(Serie, imdbID = imdb_id)    
    if request.method=='POST':
        serie.delete()        
        return HttpResponseRedirect(reverse('serie:manager'))
